...
import com.sisllc.instaiml.repository.TestUserRepository;
...
public class DataGeneratorService extends DataGeneratorBase {
    ...
    private final MedicalDocumentRepository medDocumentRepository;
    ...
    public void seedDataTestUserOnly() {
        log.info("seedDataUserOnly testUsers count {} ...", this.testUsers.size());
        generateTestUsers()
                .doOnComplete(() -> log.info("generateTestUsers complete"))
                .doOnError(e -> log.error("generateTestUsers error", e))
                .subscribe();
        log.info("Done seedDataTestUserOnly testUsers count {}", this.testUsers.size());
    }

    public void seedData() {
        log.info("seedData testUsers count {} ...", this.testUsers.size());
        this.seedDataAll();
    ...
    public void seedDataAll() {
        ...
        log.info("seedDataAll ... testUsers={}", this.testUsers.size());
    ...
    public Mono<Void> generateAllData() {
        ...
        return generateTestUsers()
                .collectList()
                .doOnNext(list -> this.testUsers = list)
            ...
            .then(generateMedicalDocuments().collectList()
                .doOnNext(list -> this.medicalDocuments = list)
                .doOnError(e -> log.error("generateMedicalDocuments error", e))
            )
    ...
    public void seedDataReactive() {
        ...
        generateMedicalDocuments()
            .doOnComplete(() -> log.info("generateMedicalDocuments complete"))
            .doOnError(e -> log.error("generateMedicalDocuments error", e))
            .subscribe();
    ...
    public void retrieveAllData() {
        this.testUserRepository.findAll().collectList()
                .subscribe(c -> this.testUsers = c);
        ...
        this.medDocumentRepository.findAll().collectList()
            .subscribe(c -> this.medicalDocuments = c);
    ...
    public void retrieveAllDataForDebug() {
        this.testUsers = this.testUserRepository.findAll().collectList()
                .block();
        ...
        this.medicalDocuments = this.medDocumentRepository.findAll().collectList()
            .block();
    ...
    private Flux<TestUser> generateTestUsers() {
        return tx.transactional(Flux.range(0, 50)
                .map(i -> TestUserGenerator.generate(getUPWD(i), passwordEncoder))
                .flatMap(rec -> template.insert(TestUser.class)
                .using(rec)
                .doOnError(e -> log.error("Error TestUser: {}", e.getMessage()))
                .thenReturn(rec)
                )
                .collectList()
                .doOnSuccess(saved -> {
            this.testUsers = saved;
            log.info("TestUser {}", saved.size());
                })
    ...
    private Flux<MedicalDocument> generateMedicalDocuments() {
        return tx.transactional(
            Flux.range(0, 50)
                .map(i -> MedicalDocumentGenerator.generate())
                .flatMap(rec -> template.insert(MedicalDocument.class)
                .using(rec)
                .doOnError(e -> log.error("Error MedicalDocument: {}", e.getMessage()))
                .thenReturn(rec)
                )
                .collectList()
                .doOnSuccess(saved -> {
                this.medicalDocuments = saved;
                log.info("MedicalDocument {}", saved.size());
                })
                .flatMapMany(Flux::fromIterable)
        );
    }
    ...
    public void seedDataByDbClient() {
        ...
        log.info("seedDataByDbClient users {}", this.testUsers.size());
        generateTestUsersByDbClient();
        ...
        log.info("Done seedDataByDbClient users {} claimsData {}", this.testUsers.size(), this.claimsData.size());
    }

    private void generateTestUsersByDbClient() {
        testUsers = IntStream.range(0, 50)
                .mapToObj(i
                        -> TestUserGenerator.generate(dbClient, getUPWD(i), passwordEncoder)
                )
            .collect(Collectors.toList());
        log.info("generateUsers DONE {}", testUsers.size());
    }
